...
export default function Services() {
  ...
  return (
    <main className="w-full bg-[#FAFAFA] relative overflow-hidden py-24 selection:bg-blue-100">
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        ...
        <div className="mb-24 text-center">
          <motion.div
            ...
            className="flex items-center gap-3 text-[#4A8EBC] mb-6 justify-center"
          >
            <div className="w-12 h-[2px] bg-[#4A8EBC]" />
            <span className="text-xs font-bold uppercase tracking-[0.25em]">Capabilities</span>
          </motion.div>

          <motion.h1
            initial={{ opacity: 0, y: 20 }}
            whileInView={{ opacity: 1, y: 0 }}
            className="text-5xl md:text-6xl font-black text-neutral-900 tracking-tighter leading-tight"
          >
        ...
        <motion.div
          ...
          className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-6 lg:gap-10"
        >
          <SpotlightCard
            icon={<Cpu size={32} />}
          ...
          <motion.div
            ...
            className="relative bg-gradient-to-br from-[#1A2A44] to-[#0F172A] rounded-[2.5rem] p-12 text-white flex flex-col justify-center items-center text-center shadow-2xl overflow-hidden group border border-white/10"
          >
            ...
            <div className="absolute top-0 right-0 w-64 h-64 bg-[#4A8EBC]/20 rounded-full blur-[80px] -translate-y-1/2 translate-x-1/2" />
...
function SpotlightCard({ icon, title, desc, features }) {
  ...
  return (
    <motion.div
      ...
      className="group relative border border-neutral-200 bg-white rounded-[2.5rem] p-8 md:p-10 shadow-sm hover:shadow-xl transition-shadow duration-500 overflow-hidden"
    ...
    >
      ...
      <div className="relative z-10 h-full flex flex-col">
        <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-[#4A8EBC]/10 to-blue-500/5 text-[#4A8EBC] flex items-center justify-center mb-8 border border-[#4A8EBC]/10 group-hover:scale-110 transition-transform duration-500">
          {icon}
        </div>

        <h3 className="text-3xl font-black text-neutral-900 mb-4 tracking-tight">{title}</h3>
        <p className="text-neutral-500 text-base mb-10 font-medium leading-relaxed">{desc}</p>
        ...
        <div className="mt-auto space-y-3">
          {features.map((f, i) => (
            <motion.div
              key={i}
              whileHover={{ x: 6 }}
              className="flex items-center justify-between p-3 rounded-xl bg-neutral-50 border border-transparent hover:border-[#4A8EBC]/20 hover:bg-white transition-all cursor-default group/item"
            >
              <div className="flex items-center gap-3">
                <span className="text-[#4A8EBC]/70 group-hover/item:text-[#4A8EBC]">{f.i}</span>
                <span className="text-sm font-bold text-neutral-600 group-hover/item:text-neutral-900">{f.t}</span>
              </div>
            </motion.div>
          ))}
        </div>
