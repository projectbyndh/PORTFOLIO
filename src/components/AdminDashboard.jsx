...
export default function AdminDashboard() {
  ...
  // Declare navigationButtons at the top level of the component
  const navigationButtons = [
    { label: 'Dashboard', icon: Home, onClick: () => setActiveTab('dashboard') },
    { label: 'Blogs', icon: FileText, onClick: () => setActiveTab('blogs') },
    { label: 'New Blog', icon: PlusCircle, onClick: () => navigate('/admin/blog/create') },
    { label: 'Careers', icon: Briefcase, onClick: () => navigate('/admin/careers') },
    ...
    <div className="min-h-screen bg-linear-to-br from-slate-50 via-white to-slate-100 flex">
      ...
      <aside className="hidden lg:flex lg:flex-col w-64 bg-white border-r border-slate-200 shadow-sm">
        <div className="p-6 border-b border-slate-200">
          <h2 className="text-2xl font-bold bg-linear-to-r from-[#26a8df] to-[#1e7ba8] bg-clip-text text-transparent">
            Admin Panel
          </h2>
        ...
        <nav className="flex-1 p-4 space-y-2">
          {renderSidebarButtons(navigationButtons)}
        </nav>
      ...
      >
        ...
        <nav className="flex-1 p-4 space-y-2">
          {renderSidebarButtons(navigationButtons, navigate, setActiveTab, activeTab)}
        </nav>
      </aside>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <main className="flex-1 p-3 sm:p-4 lg:p-6">
          ...
          {activeTab === 'blogs' && (
            <div className="bg-white/70 backdrop-blur-sm border border-white/50 rounded-xl sm:rounded-2xl shadow-xl">
                          ...
                          safeBlogs.map((blog) => (
                            <tr key={blog._id || blog.id} className="border-b border-[#26a8df]/10 hover:bg-[#26a8df]/5 transition-colors duration-200">
                              ...
                              <td className="py-2 sm:py-3 lg:py-4 px-2 sm:px-3 lg:px-4 hidden lg:table-cell">
                                <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${blog._id?.startsWith('local-')
                              ...
                              <td className="py-2 sm:py-3 lg:py-4 px-2 sm:px-3 lg:px-4">
                                <div className="flex items-center justify-end gap-0.5 sm:gap-1">
                                  <button
                                    ...
                                    className="p-1.5 sm:p-2 text-blue-600 hover:bg-blue-50 rounded-lg transition-colors duration-200 group relative"
                                    title="View Blog"
                                  >
                                  ...
                                  <button
                                    ...
                                    className="p-1.5 sm:p-2 text-green-600 hover:bg-green-50 rounded-lg transition-colors duration-200 group relative"
                                    title="Edit Blog"
                                  >
                                  ...
                                  <button
                                    ...
                                    className="p-1.5 sm:p-2 text-red-600 hover:bg-red-50 rounded-lg transition-colors duration-200 group relative disabled:opacity-50 disabled:cursor-not-allowed"
                                    title="Delete Blog"
                                  >
                                    {deleteLoading === (blog._id || blog.id) ? (
                                      <div className="w-3.5 h-3.5 sm:w-4 sm:h-4 border-2 border-red-600 border-t-transparent rounded-full animate-spin"></div>
                                    ) : (
...
// Pass variables explicitly to renderSidebarButtons
const renderSidebarButtons = (buttons, navigate, setActiveTab, activeTab) =>
  buttons.map((button, index) => (
    <button
      key={index}
      onClick={() => {
        if (button.label.toLowerCase() === 'new blog' || button.label.toLowerCase() === 'careers') {
          navigate(button.onClick);
        } else {
          setActiveTab(button.label.toLowerCase());
        }
      }}
      className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg font-medium transition-all ${
        activeTab === button.label.toLowerCase()
...
// Update the call to renderSidebarButtons
<nav className="flex-1 p-4 space-y-2">
  {renderSidebarButtons(navigationButtons)}
</nav>
