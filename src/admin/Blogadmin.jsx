...
const AdminPanel = () => {
  ...
  const handleEdit = (blog) => {
    ...
    setImage(null); // Reset image input for editing
  };
    ...
    <div className="min-h-screen bg-gradient-to-b from-[#F5FAFF] to-[#EAF5FF] font-sans text-[#1A2A44] relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto p-6 py-16 relative z-10">
        ...
        <div className="mb-12 p-6 bg-white/50 backdrop-blur-sm rounded-xl shadow-lg hover:shadow-xl transition-shadow duration-300">
          ...
          <form onSubmit={handleViewById} className="flex space-x-4">
            <input
              type="text"
              value={viewId}
          ...
          {selectedBlog && (
            <div className="mt-6 p-6 bg-white rounded-lg shadow-inner hover:transform hover:-translate-y-2 transition-all duration-300">
              {selectedBlog.image && (
                <img
                  ...
                  onError={(e) => {
                    e.target.src = '/placeholder.svg?height=192&width=768';
                  }}
                />
              )}
              ...
              <p className="text-[#2B4066]/80 mt-2">{selectedBlog.content}</p>
              <p className="text-sm text-[#2B4066]/60 mt-2">ID: {selectedBlog._id}</p>
            </div>
          )}
        </div>
